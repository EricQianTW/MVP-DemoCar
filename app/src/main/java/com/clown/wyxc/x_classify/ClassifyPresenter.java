...
public class ClassifyPresenter implements ClassifyContract.Presenter {
    private final ClassifyContract.View mView;

    public ClassifyPresenter(@NonNull ClassifyContract.View loginView){
    ...
    @Override
    public void getGoodsType(String param){
        try {
            OkHttpUtils
                    .postString()
                    .url("http://api.ixiuc.com//api/Goods/GetGoodsType")
                    ...
                    .execute(new Callback<String>() {
                        ...
                        public void onResponse(String response) {
                            try {
                                TypeToken<Message<List<GoodsTypeResult>>> token = new TypeToken<Message<List<GoodsTypeResult>>>() {
                                };
                                Message<List<GoodsTypeResult>> dataPackage = GSONUtils.fromJson(response, token);
                                if (dataPackage.getStatusCode() == Constants.OKHTTP_RESULT_SUCESS) {
                                    mView.setGetGoodsTypeResult(dataPackage.getBody());
                                } else {
