...
public class GoodsDetailPresenter_Assess implements GoodsDetailContract_Assess.Presenter {
    ...
    @Override
    public void getMerchantEvaluate(String param){
        try {
            OkHttpUtils
                    .postString()
                    .url("http://api.ixiuc.com/api/Evaluate/GetMerchantEvaluate")
                    ...
                    .execute(new Callback<String>() {
                        ...
                        public void onResponse(String response) {
                            try {
                                TypeToken<Message<List<MerchantEvaluateResult>>> token = new TypeToken<Message<List<MerchantEvaluateResult>>>() {
                                };
                                Message<List<MerchantEvaluateResult>> dataPackage = GSONUtils.fromJson(response, token);
                                if (dataPackage.getStatusCode() == Constants.OKHTTP_RESULT_SUCESS) {
                                    mView.setGetMerchantEvaluateResult(dataPackage.getBody());
                                } else {
