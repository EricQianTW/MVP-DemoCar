...
public class FeedbackPresenter extends BasePresenter implements FeedbackContract.Presenter{
    ...
    public FeedbackPresenter(@NonNull FeedbackContract.View loginView){
    ...
    @Override
    public void postFeedBack(String param){
        try {
            OkHttpUtils
                    .postString()
                    .url("http://api.ixiuc.com//api/FeedBack/PostFeedBack")
                    ...
                    .execute(new Callback<String>() {
                        ...
                        public void onResponse(String response) {
                            try {
                                ...
                                if (dataPackage.getStatusCode() == Constants.OKHTTP_RESULT_SUCESS) {
                                    mView.setPostFeedBackResult(dataPackage.getBody());
                                } else {
                                    mView.showError(dataPackage.getCustomCode(), dataPackage.getInfo());
                                    Logger.e(TAG, dataPackage.getInfo());
                                }
